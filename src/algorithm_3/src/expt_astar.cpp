...
int  cells_per_meter = 5; //Gives grid resolution
...
void tracePath(vector<vector<cell>> cellDetails, Pair dest)
{
  ...
  while (!Path.empty()) {
    ...
    double lat_meters = p.first/cells_per_meter;
    double long_meters = p.second/cells_per_meter;
...
int main(int argc, char** argv)
{
  ...
  ROW = roundup(ygn)*cells_per_meter;
  COL = roundup(xgn)*cells_per_meter;
